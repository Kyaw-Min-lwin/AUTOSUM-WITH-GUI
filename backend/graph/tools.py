...
# ==========================================
@tool
def check_path_feasibility(start_x: float, start_y: float, target_x: float, target_y: float, obstacles: list) -> str:
    """
...
# ==========================================
@tool
def calculate_spatial_relationship(robot_x: float, robot_y: float, target_x: float, target_y: float) -> str:
    """
    ...
    # Your quadrant logic
    if dx >= 0 and dy >= 0: quadrant = "North-East"
    elif dx < 0 and dy >= 0: quadrant = "North-West"
    elif dx < 0 and dy < 0: quadrant = "South-West"
...
# ==========================================
@tool
def dispatch_physical_action(skill_name: str, target_id: str = None, leader_id: str = None) -> str:
    """
    Use this tool when you have made your final decision on what physical action to take.
    Valid skill_names: 'GoToTargetSkill', 'WanderSkill', 'FollowLeaderSkill', 'SpinScanSkill'.
    If GoToTargetSkill, provide target_id. If FollowLeaderSkill, provide leader_id.
    """
    
    payload = {"skill": skill_name, "parameters": {}}
    if target_id:
        payload["parameters"]["target_id"] = target_id
    if leader_id:
        payload["parameters"]["leader_id"] = leader_id
